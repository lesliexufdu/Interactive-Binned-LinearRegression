...
#
# Last Modified: 2023-07-19 04:00:25
#
...
def feature2woe(feature, bins, woe_table, if_categorical=True):
    ...
    if if_categorical: # 分类型变量
        woe_data = feature.apply(categorical_feature_str, bins=bins).map(lambda x:woe_map[x]).to_numpy()
    else:
        cut_bins = [-np.inf,*bins,np.inf]
        cut_labels = [("-",bins[0])] + [(i,j) for i,j in zip(bins[:-1],bins[1:])] + [(bins[-1],"+")]
        woe_data = pd.cut(feature, bins=cut_bins, labels=cut_labels, duplicates='drop').map(lambda x:woe_map[x]).to_numpy()
    return woe_data
...
def data_prepare(project_path, project_config, used_features, features_config, mode="train"):
    ...
    df = pd.read_csv(
        data_path,
        sep="\t",
        usecols=[*used_features,project_config['target']],
        dtype={i:str for i in used_features if i in project_config["categorical features"]}
    )
